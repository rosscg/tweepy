...
class RateLimitHandler(OAuthHandler):
   ...
   def _load_rate_limit_status(self, key, rate_limit_status):
       ...
       self.tokens[key]['resources'] = dict(
           [(clean_path(r), rd[r]) \
           for c, rd in rate_limit_status['resources'].items() \
           for r in rd.keys()]
       )
   ...
   def select_access_token(self, resource):
       ...
       if not self.fixed_access_token: # autoselect
            ...
            #TODO: Have changed this from old index method
            key = self.access_token or list(self.tokens)[0]# current
            ...
            #TODO: Remove Iteritems x2 (done)
            if remaining == 0:
               key, limits = max(
                   [(k, sr['resources'].get(resource, self.nolimits)) \
                   for k, sr in self.tokens.items()],
